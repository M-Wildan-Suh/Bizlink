    ...
    <div class="sm:pl-12 sm:pr-12 lg:pr-32 duration-300 pt-8 pb-20 sm:pb-8 px-4 space-y-6">
        <div class="w-full bg-neutral-100 rounded-md shadow-md shadow-black/20 relative overflow-hidden">
            <div id="background" class=" absolute inset-0 flex items-center justify-center">
                ...
                <img style="display: {{old('bg_type', $template->bg_type ?? null) === "image" ? "block" : "none"}}" id="bg_image-now" src="{{asset('assets/images/placeholder.webp')}}" class=" w-full h-full object-cover object-center" alt="">
            </div>
            <form action="{{ route('template.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class=" relative w-full p-4 sm:p-6 bg-white">
                    <x-admin.component.textinput title="Name" placeholder="Input Template Name..." :value="old('name')" name="name" />
                </div>
                <div class=" space-y-4 sm:space-y-6 relative  p-4 sm:p-6">
                    <div class=" w-full">
                        <div class="w-full flex items-center justify-center">
                            <div class=" w-[400px] aspect-[3/2] max-h-full max-w-full rounded-md overflow-hidden shadow-md shadow-black/20 relative">
                                ...
                                <div class=" w-full">
                                    <img id="header" src="{{asset('assets/images/template/header/'.old('header', 'one').'.jpg')}}" class=" w-full" alt="">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class=" w-full">
                        <div class="w-full flex items-center justify-center">
                            <div class=" w-[400px] rounded-md overflow-hidden relative">
                                ...
                                <div class=" w-full">
                                    <img id="gallery" src="{{asset('assets/images/template/gallery/'.old('gallery', 'one').'.png')}}" class=" w-full" alt="">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class=" w-full">
                        <div class=" w-full flex items-center justify-center">
                            <div id="desc" style="background-color: {{old('desc_main_color') ?? 'white'}}; color: {{old('desc_text_color' ?? 'black')}}" class=" max-w-[400px] w-full rounded-md shadow-md p-4 space-y-2 overflow-hidden relative">
                                @include('components.admin.template.article')
                                <div class=" w-full flex flex-wrap gap-2">
                                    <a href="{{route('category', ['category' => 'makam-svargaloka'])}}">
                                        <button id="tagdesc" style="background-color: {{old('desc_second_color') ?? '#1d588d'}}" class=" px-2 py-1 text-xs text-white rounded-md">Makam Svargaloka</button>
                                    </a>
                                </div>
                                <p class="text-lg font-bold">Biaya Pemakaman Premium Dekat Di Jogja Svargaloka Memorial Garden</p>
                                <div class=" flex gap-4 items-center text-sm">
                                    <div class=" flex gap-1.5 items-center">
                                        <div id="descicon" style="color: {{old('desc_second_color') ?? '#1d588d'}}" class=" w-4 aspect-square">
                                ...
                                <style>
                                    .article a {
                                        font-weight: 700;
                                        color: {{old('desc_second_color') ?? '#1d588d'}};
                                    }
                                </style>
                            </div>
                        </div>
