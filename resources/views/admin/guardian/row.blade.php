@foreach ($data as $item)
    <tr class="{{ $data->currentPage() == 1 ? ($loop->even ? 'bg-neutral-100' : 'bg-neutral-200') : ($loop->even ? 'bg-neutral-200' : 'bg-neutral-100')  }} h-10 text-neutral-600 divide-x-2 divide-white">
        ...
        <td class="px-2 sm:px-4 py-1 min-h-10 text-center hidden lg:table-cell">
            @if (!empty($item->use_cpanel))
                <div class="font-semibold">{{ $item->cpanelAccount->name ?? 'cPanel' }}</div>
            @else
                ...
                @if ($item->url != 'bizlink.sites.id')
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open"
                            ...
                            aria-label="Edit guardian {{ $item->url }}">
                        ...
                        <div x-show="open" @click.outside="open = false"
                            class="absolute z-20 mt-1 right-0 bottom-0 bg-white border rounded shadow-lg w-44 overflow-hidden">
                            <a href="{{ route('guardian.show', ['guardian' => $item->id]) }}"
                                class="block px-4 py-2 text-sm hover:bg-gray-100">
                                Edit Domain
                            </a>
                            @if (!empty($item->use_cpanel) && $item->id !== -1)
                                <form action="{{ route('guardian.cpanel.create-domain', ['guardian' => $item->id]) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">
                                        Create Domain
                                    </button>
                                </form>
                                @if (!empty($item->cpanel_domain_created_at))
                                    <a href="{{ route('guardian.cpanel.files', ['guardian' => $item->id]) }}"
                                        class="block px-4 py-2 text-sm hover:bg-gray-100">
                                        File Manager
                                    </a>
                                @endif
                    ...
                    {{-- Delete --}}
                    <div x-data="{ open: false, deletemodal: false }" class="relative w-5 h-5">
                        <button @click="open = !open"
                        ...
                        <div x-show="open" @click.outside="open = false"
                            class="absolute z-20 mt-1 right-0 bottom-0 bg-white border rounded shadow-lg w-44 overflow-hidden">
                            @if (!empty($item->use_cpanel) && $item->id !== -1)
                                <form action="{{ route('guardian.cpanel.delete-domain', ['guardian' => $item->id]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">
                                        Delete Domain
                            ...
                            @endif
                            <button @click="open = false; deletemodal = true"
    ...
    <tr x-show="openedIds.includes({{ $item->id }})" class="{{ $loop->even ? 'bg-neutral-100' : 'bg-neutral-200' }} md:hidden border-t-2 border-b-2 border-white h-10 text-neutral-600 divide-x-2 divide-white">
        <td colspan="7" class=" pl-12 p-2">
            <div class=" w-full flex flex-col gap-2 text-sm">
                <p>Integrasi cPanel : {{ !empty($item->use_cpanel) ? ($item->cpanelAccount->name ?? 'Aktif') : 'Manual' }}</p>
