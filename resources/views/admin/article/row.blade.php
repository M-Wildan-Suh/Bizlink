@foreach ($data as $item)
    <tr class="{{ $loop->even ? 'bg-neutral-100' : 'bg-neutral-200' }} h-10 text-neutral-600 divide-x-2 divide-white">
        ...
        <td class="px-3 py-1 text-center text-nowrap hidden md:table-cell">
            @if ($item->guardian)
                <a href="https://{{ $item->guardian->url }}" class=" hover:text-byolink-1 duration-300 line-clamp-1"
                    target="__blank">{{ $item->guardian->url }}</a>
            @else
                Main
            @endif
            ...
            <div class="flex gap-1 sm:gap-2 justify-center">
                <button @click="detail({{ $item->id }})"
                    ...
                    <svg id="Layer_1" style="enable-background:new 0 0 512 512;" version="1.1" viewBox="0 0 512 512"
                        class=" w-full h-full" xml:space="preserve" xmlns="http://www.w3.org/2000/svg"
    ...
    <tr x-show="openedIds.includes({{ $item->id }})"
        class="{{ $loop->even ? 'bg-neutral-100' : 'bg-neutral-200' }} md:hidden border-t-2 border-b-2 border-white h-10 text-neutral-600 divide-x-2 divide-white">
        <td colspan="3" class=" pl-12 p-2">
            <div class=" w-full flex flex-col gap-2 text-sm">
                ...
                <p class=" flex gap-1">Guardian :
                    @if ($item->guardian)
                        <a href="https://{{ $item->guardian->url }}"
                            class=" hover:text-byolink-1 duration-300 line-clamp-1"
                            target="__blank">{{ $item->guardian->url }}</a>
                    @else
                        Main
                    @endif
