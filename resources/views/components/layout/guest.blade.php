...
<body class="font-sans antialiased" x-data="{ loading: true }" x-init="if (document.readyState === 'complete') { loading = false } else { window.addEventListener('load', () => loading = false, { once: true }) }"
    @beforeunload.window="loading = true" @pageshow.window="loading = false">
    <!-- Loading overlay -->
    ...
    <div class=" pt-[70px] sm:pt-[76px] min-h-screen">
        {{ $slot }}
    </div>
</body>
@if (env('APP_DEPLOY') == true)
